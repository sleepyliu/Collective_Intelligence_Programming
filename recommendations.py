...
def sim_pearson(prefs,p1,p2):
	...
	ave1 = sum([prefs[p1][it] for it in si])/n
	ave2 = sum([prefs[p2][it] for it in si])/n

	std1 = sqrt(sum([pow(prefs[p1][it] - ave1,2) for it in si]))
	std2 = sqrt(sum([pow(prefs[p2][it] - ave2,2) for it in si]))

	cov = sum([(prefs[p1][it] - ave1)*(prefs[p2][it] -ave2) for it in si])
